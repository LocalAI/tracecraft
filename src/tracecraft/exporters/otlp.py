...
Exports TraceCraft runs to any OTLP-compatible backend (Jaeger, Grafana Tempo,
...
class OTLPExporter(BaseExporter):
    ...
    Converts TraceCraft runs and steps to OpenTelemetry spans and exports
    ...
    def _create_span_exporter(self) -> Any:
        ...
        try:
            ...
            logger.warning(
                ...
                "Install with: pip install tracecraft[otlp]"
            )
            return None
    ...
    def _step_to_span_data(self, step: Step, trace_id: UUID) -> dict[str, Any]:
        ...
        attributes["tracecraft.step.type"] = step.type.value
        if step.error:
