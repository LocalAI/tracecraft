"""
Configuration management for TraceCraft.
...
@dataclass
class TraceCraftConfig:
    """Main configuration for TraceCraft."""
...
def load_config_from_env() -> TraceCraftConfig:
    """
    Load configuration from environment variables.

    Environment variables:
        TRACECRAFT_SERVICE_NAME: Service name
        TRACECRAFT_CONSOLE_ENABLED: Enable console exporter (true/false)
        TRACECRAFT_JSONL_ENABLED: Enable JSONL exporter (true/false)
        TRACECRAFT_JSONL_PATH: Path for JSONL file
        TRACECRAFT_SAMPLING_RATE: Sampling rate (0.0-1.0)
        TRACECRAFT_OTLP_ENABLED: Enable OTLP exporter (true/false)
        TRACECRAFT_OTLP_ENDPOINT: OTLP collector endpoint
        TRACECRAFT_REDACTION_ENABLED: Enable PII redaction (true/false, default: true)
        TRACECRAFT_AZURE_FOUNDRY_ENABLED: Enable Azure AI Foundry (true/false)
        TRACECRAFT_AZURE_CONTENT_RECORDING: Enable content recording (true/false)
        TRACECRAFT_AZURE_AGENT_NAME: Agent name for Azure Foundry
        TRACECRAFT_AZURE_AGENT_ID: Agent ID for Azure Foundry
        TRACECRAFT_AWS_AGENTCORE_ENABLED: Enable AWS AgentCore (true/false)
        TRACECRAFT_AWS_XRAY_PROPAGATION: Use X-Ray propagation (true/false)
        TRACECRAFT_AWS_SESSION_ID: Session ID for AgentCore
        TRACECRAFT_GCP_VERTEX_ENABLED: Enable GCP Vertex Agent (true/false)
        TRACECRAFT_GCP_SESSION_ID: Session ID for Vertex Agent
        TRACECRAFT_GCP_AGENT_NAME: Agent name for Vertex Agent
        TRACECRAFT_GCP_AGENT_ID: Agent ID for Vertex Agent
    ...
    Returns:
        TraceCraftConfig instance.
    """
    # Main config
    service_name = os.environ.get("TRACECRAFT_SERVICE_NAME", "tracecraft")

    console_enabled_str = os.environ.get("TRACECRAFT_CONSOLE_ENABLED")
    console_enabled = _parse_bool(console_enabled_str) if console_enabled_str else True

    jsonl_enabled_str = os.environ.get("TRACECRAFT_JSONL_ENABLED")
    jsonl_enabled = _parse_bool(jsonl_enabled_str) if jsonl_enabled_str else True

    jsonl_path = os.environ.get("TRACECRAFT_JSONL_PATH")

    # Sampling config
    sampling_rate_str = os.environ.get("TRACECRAFT_SAMPLING_RATE")
    sampling_rate = 1.0
    if sampling_rate_str:
        with contextlib.suppress(ValueError):
            sampling_rate = float(sampling_rate_str)

    always_keep_errors_str = os.environ.get("TRACECRAFT_SAMPLING_KEEP_ERRORS")
    ...
    # Exporter config
    otlp_enabled_str = os.environ.get("TRACECRAFT_OTLP_ENABLED")
    otlp_enabled = _parse_bool(otlp_enabled_str) if otlp_enabled_str else False

    otlp_endpoint = os.environ.get("TRACECRAFT_OTLP_ENDPOINT")
    ...
    redaction_enabled_str = os.environ.get("TRACECRAFT_REDACTION_ENABLED")
    ...
    azure_enabled_str = os.environ.get("TRACECRAFT_AZURE_FOUNDRY_ENABLED")
    azure_enabled = _parse_bool(azure_enabled_str) if azure_enabled_str else False

    azure_content_recording_str = os.environ.get("TRACECRAFT_AZURE_CONTENT_RECORDING")
    ...
    azure_foundry_config = AzureFoundryConfig(
        ...
        agent_name=os.environ.get("TRACECRAFT_AZURE_AGENT_NAME"),
        agent_id=os.environ.get("TRACECRAFT_AZURE_AGENT_ID"),
    )

    # AWS AgentCore config
    aws_enabled_str = os.environ.get("TRACECRAFT_AWS_AGENTCORE_ENABLED")
    aws_enabled = _parse_bool(aws_enabled_str) if aws_enabled_str else False

    aws_xray_str = os.environ.get("TRACECRAFT_AWS_XRAY_PROPAGATION")
    ...
    gcp_enabled_str = os.environ.get("TRACECRAFT_GCP_VERTEX_ENABLED")
    gcp_enabled = _parse_bool(gcp_enabled_str) if gcp_enabled_str else False

    gcp_content_recording_str = os.environ.get("TRACECRAFT_GCP_CONTENT_RECORDING")
...
def load_config(**kwargs: Any) -> TraceCraftConfig:
    """
