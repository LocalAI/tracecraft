"""
MLflow storage backend for TraceCraft.
...
class MLflowTraceStore(BaseTraceStore):
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError("mlflow required. Install with: pip install tracecraft[mlflow]")
    ...
    def save(self, run: AgentRun, project_id: str | None = None) -> None:  # noqa: ARG002
        ...
        with self._mlflow.start_run(run_name=run.name):
            ...
            self._mlflow.set_tag("tracecraft.run_id", str(run.id))
            self._mlflow.set_tag("tracecraft.name", run.name)
            if run.environment:
                self._mlflow.set_tag("tracecraft.environment", run.environment)
            if run.session_id:
                self._mlflow.set_tag("tracecraft.session_id", run.session_id)
            if run.user_id:
                self._mlflow.set_tag("tracecraft.user_id", run.user_id)
            if run.error:
                self._mlflow.set_tag("tracecraft.has_error", "true")
            for tag in run.tags:
                self._mlflow.set_tag(f"tracecraft.tag.{tag}", "true")
    ...
    def get(self, trace_id: str) -> AgentRun | None:
        ...
        Searches for MLflow run with matching tracecraft.run_id tag.
        """
        filter_string = f"tags.`tracecraft.run_id` = '{trace_id}'"
    ...
    def query(self, query: TraceQuery) -> list[AgentRun]:
        ...
        if query.name:
            filter_parts.append(f"tags.`tracecraft.name` = '{query.name}'")

        if query.name_contains:
            filter_parts.append(f"tags.`tracecraft.name` LIKE '%{query.name_contains}%'")
        ...
        if query.environment:
            filter_parts.append(f"tags.`tracecraft.environment` = '{query.environment}'")

        if query.session_id:
            filter_parts.append(f"tags.`tracecraft.session_id` = '{query.session_id}'")

        if query.user_id:
            filter_parts.append(f"tags.`tracecraft.user_id` = '{query.user_id}'")
    ...
    def _load_trace_artifact(self, mlflow_run_id: str) -> AgentRun | None:
        ...
        try:
            ...
            for artifact_name in ["trace.json", "tracecraft.json"]:
                try:
    ...
    def search(self, filter_string: str) -> list[AgentRun]:
        ...
        Example:
            traces = store.search(
                "metrics.duration_ms > 1000 AND tags.`tracecraft.name` LIKE '%research%'"
            )
        """
        try:
            runs = self._mlflow.search_runs(
