"""
Claude Agent SDK adapter for AgentTrace.

Provides ClaudeAgentTracer that integrates with Claude SDK hooks
to capture agent execution as AgentTrace Steps.
"""
...
class ClaudeAgentTracer:
    """
    Tracer for Claude Agent SDK that creates AgentTrace Steps from hooks.

    Integrates with the Claude SDK's hook system (PreToolUse, PostToolUse,
    Stop, SubagentStop) to capture agent execution as AgentTrace Steps.
    ...
    def get_hooks(self) -> dict[str, list[Any]]:
        ...
        These hooks are used to intercept tool execution and create AgentTrace Steps.
        ...
        try:
            ...
            raise ImportError(
                "claude-agent-sdk is required for Claude SDK integration. "
    ...
    def get_options(self, **kwargs: Any) -> Any:
        ...
        return ClaudeAgentOptions(hooks=merged_hooks, **kwargs)
