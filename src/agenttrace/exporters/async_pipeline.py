"""
Async export pipeline for AgentTrace.
    ...
    def _start_workers(self) -> None:
        ...
        for i in range(self._num_workers):
            worker = threading.Thread(
                ...
                name=f"agenttrace-async-exporter-{i}",
                daemon=True,
            )
            worker.start()
...
class AsyncBatchExporter(BaseExporter):
    ...
    def __init__(
        ...
        self._worker = threading.Thread(
            target=self._worker_loop,
            name="agenttrace-batch-exporter",
            daemon=True,
        )
        self._worker.start()
...
class AsyncioExporter(BaseExporter):
    ...
    Example:
        import asyncio
        from agenttrace.exporters.otlp import OTLPExporter
        from agenttrace.exporters.async_pipeline import AsyncioExporter
