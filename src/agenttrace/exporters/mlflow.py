...
Exports AgentTrace runs to MLflow Tracking for visualization,
...
class MLflowExporter(BaseExporter):
    ...
    Example:
        ...
        agenttrace.init(exporters=[mlflow_exporter])
        ```
    """
    ...
    def __init__(
        ...
        experiment_name: str = "agenttrace",
        run_name_prefix: str = "",
    ...
    def _setup_mlflow(self) -> None:
        ...
        try:
            ...
            logger.warning("mlflow not installed. Install with: pip install agenttrace[mlflow]")
    ...
    def _log_run_params(self, run: AgentRun) -> None:
        ...
        self._mlflow.log_param("agenttrace.run_id", str(run.id))
        self._mlflow.log_param("agenttrace.name", run.name)

        if run.description:
            self._mlflow.log_param("agenttrace.description", run.description[:250])
        if run.session_id:
    ...
    def _log_run_tags(self, run: AgentRun) -> None:
        ...
        for tag in run.tags:
            ...
            self._mlflow.set_tag(f"agenttrace.tag.{safe_tag}", "true")

        # Log if there was an error
        if run.error:
            self._mlflow.set_tag("agenttrace.has_error", "true")
            self._mlflow.set_tag("agenttrace.error_type", run.error_type or "Unknown")
        ...
        try:
            for step in steps:
                with self._mlflow.start_span(name=step.name) as span:
                    # Set span attributes
                    span.set_attribute("agenttrace.step.id", str(step.id))
                    span.set_attribute("agenttrace.step.type", step.type.value)
    ...
    def _log_trace_artifact(self, run: AgentRun) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                self._mlflow.log_artifact(str(filepath), artifact_path="agenttrace")
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if run.input is not None:
                    ...
                    self._mlflow.log_artifact(str(input_path), artifact_path="agenttrace")
                ...
                if run.output is not None:
                    ...
                    self._mlflow.log_artifact(str(output_path), artifact_path="agenttrace")
