...
def create_cloudtrace_exporter(
    ...
    Example:
        ```python
        from agenttrace.contrib.gcp import create_cloudtrace_exporter
        import agenttrace
    ...
    Note:
        Requires google-auth package: pip install agenttrace[gcp]
...
def _get_access_token(credentials: Any | None = None) -> str:
    ...
    try:
        ...
        raise ImportError(
            "google-auth required for GCP. Install with: pip install agenttrace[gcp]"
        ) from e
...
def configure_for_cloud_run(
    ...
    Example:
        ```python
        from agenttrace.contrib.gcp import configure_for_cloud_run
        import agenttrace
    ...
    svc_name = service_name if service_name else os.environ.get("K_SERVICE", "agenttrace")
...
def configure_for_cloud_functions(
    ...
    Example:
        ```python
        from agenttrace.contrib.gcp import configure_for_cloud_functions
        import agenttrace
    ...
    svc_name = service_name if service_name else os.environ.get("FUNCTION_NAME", "agenttrace")
...
def configure_for_gke(
    ...
    Example:
        ```python
        from agenttrace.contrib.gcp import configure_for_gke
        import agenttrace
...
def configure_for_vertex_ai(
    ...
    Example:
        ```python
        from agenttrace.contrib.gcp import configure_for_vertex_ai
        import agenttrace
...
def configure_for_vertex_agent_builder(
    ...
    - AGENTTRACE_GCP_SESSION_ID: Session ID for conversation tracking
    - AGENTTRACE_GCP_AGENT_NAME: Agent name
    - AGENTTRACE_GCP_AGENT_ID: Agent ID
    ...
    if enable_content_recording is None:
        env_value = os.environ.get("AGENTTRACE_GCP_CONTENT_RECORDING", "false")
...
def inject_cloudtrace_context(
    ...
    Example:
        ...
        ```
    """
    from agenttrace.propagation.cloudtrace import CloudTraceContextPropagator
...
class VertexAITracerAdapter:
    ...
    Wraps AgentTraceCallbackHandler and adds Vertex AI-specific
    ...
    Note:
        This adapter wraps AgentTraceCallbackHandler and adds Vertex AI
