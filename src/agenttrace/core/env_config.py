"""
Environment-aware configuration for AgentTrace.
...
def detect_environment() -> str:
    ...
    1. AGENTTRACE_ENVIRONMENT / AGENTTRACE_ENV (explicit override)
    ...
    explicit = os.environ.get("AGENTTRACE_ENVIRONMENT") or os.environ.get("AGENTTRACE_ENV")
    if explicit:
...
class ExporterConfig(BaseModel):
    ...
    # MLflow settings
    mlflow_tracking_uri: str | None = None
    mlflow_experiment_name: str = "agenttrace"
...
class AgentTraceEnvConfig(BaseModel):
    """
...
def load_config(
    ...
    Args:
        ...
        env: Override environment. If None, uses AGENTTRACE_ENV or defaults to "development".
    ...
    if config_path is None:
        search_paths = [
            Path.cwd() / ".agenttrace" / "config.yaml",
            Path.cwd() / ".agenttrace" / "config.yml",
            Path.home() / ".agenttrace" / "config.yaml",
            Path.home() / ".agenttrace" / "config.yml",
        ]
    ...
    if env:
        config_data["env"] = env
    elif "AGENTTRACE_ENV" in os.environ:
        config_data["env"] = os.environ["AGENTTRACE_ENV"]

    # Expand environment variables in string values
    config_data = _expand_env_vars(config_data)

    return AgentTraceEnvConfig.model_validate(config_data)
...
_config: AgentTraceEnvConfig | None = None


def get_config() -> AgentTraceEnvConfig:
...
def set_config(config: AgentTraceEnvConfig) -> None:
