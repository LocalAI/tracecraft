...
- Check the [Glossary](/glossary) if a term is unfamiliar
- Look at the [Changelog](/changelog) if behavior changed after an upgrade
...
[Open a Discussion →](https://github.com/LocalAI/tracecraft/discussions)
...
<Callout type="info">
**Minimal Reproduction:** The single most effective way to get a quick answer is to provide the smallest possible code example that reproduces the issue. Trim your code to just what is needed; this also often reveals the root cause yourself.
</Callout>
...
[Open an Issue →](https://github.com/LocalAI/tracecraft/issues/new/choose)

### Before Opening an Issue

- Search [existing issues](https://github.com/LocalAI/tracecraft/issues) to check for duplicates
- Check the [Changelog](/changelog) to see if the bug was already fixed in a newer version
...
**Before Submitting:** Make sure your reproduction code does not contain real API keys, credentials, or personal data. Replace them with placeholder strings like `"sk-..."` or `os.environ["OPENAI_API_KEY"]`.
</Callout>
...
[Post an Idea →](https://github.com/LocalAI/tracecraft/discussions/new?category=ideas)
...
<Callout type="info">
**If Your Idea Gets Traction:** Once a discussion has community support and clear scope, a maintainer will convert it to a GitHub Issue and add it to the project roadmap.
</Callout>
...
<Callout type="error">
**Do Not Report Security Issues Publicly:** If you discover a security vulnerability - including authentication bypasses, data exposure, or dependency vulnerabilities - **do not open a public GitHub Issue or Discussion**. Public disclosure before a fix is available puts all TraceCraft users at risk.
</Callout>
...
<Callout type="info">
**Start Small:** First-time contributors often find it easiest to start with documentation improvements, test additions, or issues labeled `good first issue`. These changes are lower-risk and still genuinely useful.
</Callout>
