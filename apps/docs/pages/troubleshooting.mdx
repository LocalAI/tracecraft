...
If your problem is not listed here, check the [FAQ](/faq) or open an issue on
...
<Tabs items={['macOS (Apple Silicon)', 'Windows', 'Linux (Alpine/musl)']}>
<Tabs.Tab>
...
After calling `auto_instrument()`, OpenAI or Anthropic API calls do not appear in traces.

**Cause**

The `openai` (or `anthropic`) module was already imported before `auto_instrument()` was
...
{/* main.py */}
```python
# 1. Initialize the runtime first
import tracecraft
tracecraft.init(console=True)

# 2. Patch SDKs before importing anything that uses them
...
Treat `tracecraft.init()` and `auto_instrument()` like logging setup - they must be the
very first lines of your application entry point, before any application code is
imported.
...
Both a `@trace_llm` decorator on a wrapper function and `auto_instrument()` are active,
...
```python
# Option A: Use @trace_llm decorator, do not use auto_instrument for this SDK
...
# Option B: Use auto_instrument(), remove the decorator
async def call_gpt4(prompt: str) -> str:
...
auto_instrument(instrument=["anthropic"])  # Only Anthropic, not OpenAI
```
...
Review [Core Concepts](/getting-started/concepts) for the definition of each
...
<Callout type="warning" title="Security trade-off">
...
<Tabs items={['Update the system CA bundle', 'Specify a custom CA bundle', 'Disable TLS for internal networks only']}>
<Tabs.Tab>
